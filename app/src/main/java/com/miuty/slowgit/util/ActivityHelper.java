...
import com.miuty.slowgit.ui.screen.LinksParserActivity;
...
public class ActivityHelper {
    ...
    public static void shareUrl(@NonNull Context context, @NonNull String url) {
        ...
        try {
            ...
            Toasty.error(SlowGitApplication.getInstance(), e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
    ...
    public static boolean checkAndRequestReadWritePermission(@NonNull Activity activity) {
        if (!isReadWritePermissionIsGranted(activity)) {
            ...
            Toasty.error(SlowGitApplication.getInstance(), activity.getString(R.string.read_write_permission_explanation), Toast.LENGTH_LONG).show();
            return false;
        }
        return true;
    }

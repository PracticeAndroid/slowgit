...
import static com.miuty.slowgit.util.LinkParserHelper.API_AUTHORITY;
import static com.miuty.slowgit.util.LinkParserHelper.HOST_DEFAULT;
import static com.miuty.slowgit.util.LinkParserHelper.PROTOCOL_HTTPS;
import static com.miuty.slowgit.util.LinkParserHelper.RAW_AUTHORITY;
...
public class SchemeParser {
    ...
    @Nullable
    private static Intent getIntentForURI(@NonNull Context context, @NonNull Uri data, boolean showRepoBtn) {
    ...
    @Nullable
    private static Intent getPullRequestIntent(@NonNull Context context, @NonNull Uri uri, boolean showRepoBtn) {
        List<String> segments = uri.getPathSegments();
    ...
    @Nullable
    private static Intent getIssueIntent(@NonNull Context context, @NonNull Uri uri, boolean showRepoBtn) {
        List<String> segments = uri.getPathSegments();
        if (segments == null || segments.size() < 3) return null;
        String owner = null;
        String repo = null;
    ...
    @Nullable
    private static Intent getCommits(@NonNull Context context, @NonNull Uri uri, boolean showRepoBtn) {
    ...
    @Nullable
    private static Intent getCommit(@NonNull Context context, @NonNull Uri uri, boolean showRepoBtn) {
        List<String> segments = Stream.of(uri.getPathSegments())
                .filter(value -> !value.equalsIgnoreCase("api") || !value.equalsIgnoreCase("v3"))
                .toList();
    ...
    @Nullable
    private static Intent getReleases(@NonNull Context context, @NonNull Uri uri, boolean isEnterprise) {
        List<String> segments = uri.getPathSegments();
    ...
    private static Intent getCreateIssueIntent(@NonNull Context context, @NonNull Uri uri) {
        ...
        if (segments == null || segments.size() < 3 || !uri.getLastPathSegment().equalsIgnoreCase("new"))
            return null;
